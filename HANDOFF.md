...
**Branch:** `main`   **HEAD:** scheduler per-account publication — 2 confirmed-finding fixes commit `139ede5` (prior:
`efd0d64` release-safety corrections, `227d685` 3-blocker fixes, `f3a7d9d` per-account publication, `e4e3ff2`/`7d99d00`
scheduler-repair; report rounds 1-4 at `8285b7a`).
**139ede5 (+ efd0d64 + 227d685 + f3a7d9d + e4e3ff2) are committed locally on `main`, NOT yet pushed** — the scheduler
effect lands only on the NEXT natural cron off origin/main, so they need a push (normal path, no dispatch) to take
effect. Awaiting go-ahead to push. The healthy-subset PARTIAL publication runs a READ-ONLY schema-capability preflight
(exact `open_sync_cycle` OID via to_regprocedure + `sync_cycles_bucket_check`) BEFORE opening controls and is INERT
(zero publication writes, fail-closed) until migration `20260924` is applied (see item 3); a COMPLETE region is
unaffected.
