    ...
    <div className="container skupl-page">
      ...
      {data && !data.snapshotMissing && <>
        ...
        <StatRow stats={[
          ...
          { label: "Return rate", value: totals.rate !== null ? (totals.ratePartial ? `${fmtRate(totals.rate)} · partial` : fmtRate(totals.rate)) : (totals.ratePartial ? "unavailable · partial" : "—"), hint: totals.ratePartial ? "Partial — the rate is computed from proven rows only and EXCLUDES rows whose returned units cannot be trusted for it: currency-ambiguous ASINs (returned units withheld), rows with returns but no in-window orders, and lag-inflated rows (more returns than orders in the window). Units returned divided by ordered units." : "Units returned divided by ordered units, recomputed from the summed units" },
        ...
        <div className="footer-note">
          ...
          {" "}Return rate is the <strong>Returns-record count</strong> (one row per returned item) divided by <strong>ordered units from <code>Order Line Items</code></strong> (<code>quantity</code>) over the same window. Returns records carry no currency, so when a product's ordered/settlement evidence spans more than one currency the count cannot be attributed to a single currency and the rate is <strong>withheld</strong> rather than shown against a currency-ambiguous denominator. <code>COGS on refunded units</code> is displayed but deliberately excluded from leakage, because the source does not report whether returned stock came back sellable. Reason buckets come from <code>amazon_return_reason</code>; a cause is only named when one bucket accounts for at least half of a product's returns. Return history is limited to roughly {data.returnHistoryDays} days by the Returns source itself. Currencies are never combined. Filters, search, sorting, paging and both exports are local; only Refresh calls DataDoe, and it saves one shared snapshot for every user with access to this account.
        </div>
      </>}
    </div>
  );
