...
let priorityMod; let planMod; let brandView; let pubComposition; let reportStore; let supabaseMod; let releaseRunner;
...
const CAT_JOB_OK = [{ source_key: CATALOG, request_hash: "cat-hash", fetch_status: "succeeded" }];
...
function makeRelease(over = {}) {
  ...
  if (over.reservedHash) reservation._rows.set(OP(), { catalog_request_hash: over.reservedHash, export_id: "e1", status: "created", tokens_spent: 2 });
...
test("P4b. finalizeBucket ACCEPTS a fully-reconstructed catalog-only cycle (fresh discovery + reserved hash + org catalog job + accounts x 3 validated)", async () => {
  const rel = makeRelease({ accounts: [{ accountId: "A01" }], today: TODAY, cycle: { id: "cyc", bucket: "us", status: "running", trigger: "manual" }, reservedHash: "cat-hash", srcJobs: CAT_JOB_OK, owners: CAT_OWNERS_OK, repJobs: repJobsFor(["A01"]), finalizeResp: { disposition: "finalized", cycle: { status: "succeeded" } } });
...
test("P4c. finalizeBucket REFUSES every unrelated / open / malformed / mis-scoped condition", async () => {
  const base = { accounts: [{ accountId: "A01" }], today: TODAY, cycle: { id: "cyc", bucket: "us", status: "running", trigger: "manual" }, reservedHash: "cat-hash", srcJobs: CAT_JOB_OK, owners: CAT_OWNERS_OK, repJobs: repJobsFor(["A01"]), finalizeResp: { disposition: "finalized", cycle: { status: "succeeded" } } };
  const cases = [
    ...
    ["no reservation", { reservedHash: null }, "no-reservation"],
    ["source job count != 1", { srcJobs: CAT_JOB_OK.concat([{ source_key: CATALOG, request_hash: "cat-hash", fetch_status: "succeeded" }]) }, "source-job-count"],
    ["source not catalog", { srcJobs: [{ source_key: OLI, request_hash: "cat-hash", fetch_status: "succeeded" }] }, "source-job-not-catalog"],
    ["source hash != reserved", { srcJobs: [{ source_key: CATALOG, request_hash: "other", fetch_status: "succeeded" }] }, "source-hash-not-reserved"],
    ["source not succeeded", { srcJobs: [{ source_key: CATALOG, request_hash: "cat-hash", fetch_status: "pending" }] }, "source-job-not-succeeded"],
  ...
  for (const [name, over, reason] of cases) {
    const rel = makeRelease({ ...base, ...over });
    const res = await rel.finalizeBucket("us");
...
test("P4d. publishAccount publishes daily-reporting, brand-sales, brand-inventory -- brand-sales BEFORE brand-inventory", async () => {
  const pubCalls = [];
...
test("P6b. AFTER a terminal cycle the real publisher PUBLISHES all three", async () => {
  ...
  for (const r of outp.results) assert.equal(r.disposition, "published", r.reportKey);
});
...
function fakeRelease(over = {}) {
  return {
    publishOrder: ["daily-reporting", "brand-sales", "brand-inventory"],
    reportKeys: ["daily-reporting", "brand-sales", "brand-inventory"],
    ...
    publishAccount: over.publishAccount || (async (a) => ({ accountId: a, results: [{ reportKey: "daily-reporting", disposition: "published" }, { reportKey: "brand-sales", disposition: "published" }, { reportKey: "brand-inventory", disposition: "published" }] })),
...
function runnerDeps(over = {}) {
  return {
    ...
    readPublishGate: over.readPublishGate || (async () => ({ ready: true })),
...
test("P9f. a NOT-ready publication gate stops BEFORE any publish (no partial publish) => code 1 (publish-gates)", async () => {
  let published = 0;
  const r = await releaseRunner.runPriorityDashboardsRelease(runnerDeps({ readPublishGate: async (rk) => ({ ready: rk !== "brand-inventory", reason: rk === "brand-inventory" ? "publish_enabled=false" : null }), releaseOver: { publishAccount: async (a) => { published += 1; return { accountId: a, results: [] }; } } }));
  assert.equal(r.code, 1); assert.equal(r.stage, "publish-gates");
  assert.equal(published, 0, "no account was published when a gate was missing");
});
...
test("P9j. 'already-current' publish + 'already-terminal' finalize are accepted as success => code 0", async () => {
  const r = await releaseRunner.runPriorityDashboardsRelease(runnerDeps({ releaseOver: {
    ...
    publishAccount: async (a) => ({ accountId: a, results: [{ reportKey: "daily-reporting", disposition: "already-current" }, { reportKey: "brand-sales", disposition: "already-current" }, { reportKey: "brand-inventory", disposition: "already-current" }] }),
  } }));
