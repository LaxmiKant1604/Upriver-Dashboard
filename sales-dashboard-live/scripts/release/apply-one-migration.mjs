...
// Usage:  node scripts/release/apply-one-migration.mjs <one-frozen-filename.sql>
...
// Baseline by stage (matches runApply's internal dispatch): migration 9 -> STAGE-8 re-anchor; migration 8 ->
// STAGE-7 re-anchor; migration 7 -> STAGE-6 re-anchor; migrations 5-6 -> STAGE-4 re-anchor; migration 4 ->
// STAGE-3 re-anchor; migrations 1-3 -> stage-0.
const stageIdx = NEW6.indexOf(FILENAME);
const baselinePath = stageIdx >= 8 ? baselineStage8Path : stageIdx >= 7 ? baselineStage7Path : stageIdx >= 6 ? baselineStage6Path : stageIdx >= 4 ? baselineStage4Path : stageIdx === 3 ? baselineStage3Path : baselineStage0Path;
