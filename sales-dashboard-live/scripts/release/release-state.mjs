...
import { MIGRATIONS, NEW6, BASELINE, BASELINE_VERSION, APPROVED_IDENTITY, PROTECTED_DIGESTS, PROTECTED_DIGEST_KEYS, ALTERED_TABLE_PROJECTIONS, cumulativeAlterConstraints, cumulativeDroppedConstraints, validateIdentity, verifyMigrationPresent, verifyMigrationAbsent } from "./release-manifest.mjs";
...
export async function verifyStageObjects(q, stage) {
  const P = [];
  // The exact cumulative extra-constraint set for this stage: an ALTER-added constraint is REQUIRED on its base
  // table once its owning migration is applied (ownerIndex < stage), REJECTED as premature before that, and
  // REMOVED again once a later applied migration drops it. `dropped` also tells verifyMigrationPresent to skip an
  // add-constraint check for a constraint a later applied migration has since dropped.
  const extrasByTable = cumulativeAlterConstraints(stage);
  const dropped = cumulativeDroppedConstraints(stage);
  for (let i = 0; i < MIGRATIONS.length; i++) {
    if (i < stage) P.push(...(await verifyMigrationPresent(q, MIGRATIONS[i], extrasByTable, dropped)).map((m) => `[applied ${NEW6[i]}] ${m}`));
...
// ---- DEDICATED stage-4 re-anchor baseline (forward-only recovery from an APPLIED stage 4) -----------------
// A SEPARATE hard-coded artifact (anchorStage=4) governing ONLY migrations 5-6. The stage-0 and stage-3
// baselines are preserved untouched for audit. NOT generalized to arbitrary stages. buildStage4Baseline uses
// manifest-pinned values ONLY (never arbitrary current state).
export function buildStage4Baseline({ head, fingerprint, pins = PROTECTED_DIGESTS }) {
  return {
    version: BASELINE_VERSION, projectRef: APPROVED_IDENTITY.projectRef, head, manifestFingerprint: fingerprint,
    migrationHashes: Object.fromEntries(MIGRATIONS.map((m) => [m.file, m.sha])), anchorStage: 4,
    protectedDigest: Object.fromEntries(PROTECTED_DIGEST_KEYS.map((k) => [k, pins[k]])), // manifest-pinned values ONLY
  };
}
// Validate the reviewed stage-4 baseline. Rejects: anchorStage != exactly 4; wrong project/head/fingerprint;
// migration-hash drift; edited/missing/extra/unpinned protected digest; and (on an APPLY) a migration whose
// stage precedes the anchor (stageIdx < 4 -- migrations 1-4 are NOT governed here).
export function validateStage4Baseline(baseline, { currentHead, currentFingerprint, envRef, stageIdx, pins = PROTECTED_DIGESTS } = {}) {
  const P = [];
  if (!baseline || typeof baseline !== "object") return ["stage-4 baseline malformed (not an object)"];
  if (baseline.version !== BASELINE_VERSION) P.push(`stage-4 baseline version ${baseline.version} != ${BASELINE_VERSION}`);
  if (baseline.anchorStage !== 4) P.push(`stage-4 baseline anchorStage ${baseline.anchorStage} != 4`);
  if (baseline.projectRef !== APPROVED_IDENTITY.projectRef) P.push(`stage-4 baseline projectRef ${baseline.projectRef} != approved`);
  if (envRef != null && baseline.projectRef !== envRef) P.push(`stage-4 baseline projectRef != current env ref ${envRef} (environment switched)`);
  if (baseline.head !== currentHead) P.push(`stage-4 baseline HEAD ${baseline.head} != current HEAD ${currentHead}`);
  if (baseline.manifestFingerprint !== currentFingerprint) P.push("stage-4 baseline manifest fingerprint stale");
  if (JSON.stringify(baseline.migrationHashes) !== JSON.stringify(Object.fromEntries(MIGRATIONS.map((m) => [m.file, m.sha])))) P.push("stage-4 baseline migration hashes differ from the manifest");
  if (stageIdx != null && stageIdx < 4) P.push(`stage-4 baseline cannot govern a migration at stage ${stageIdx} (precedes anchor 4)`);
  const pd = baseline.protectedDigest;
  if (!pd || typeof pd !== "object") { P.push("stage-4 baseline protectedDigest malformed"); return P; }
  if (!setEq(Object.keys(pd), PROTECTED_DIGEST_KEYS)) P.push(`stage-4 baseline digest key set {${Object.keys(pd).sort()}} != required {${[...PROTECTED_DIGEST_KEYS].sort()}}`);
  for (const k of PROTECTED_DIGEST_KEYS) {
    const pin = pins[k], b = pd[k];
    if (!pin) { P.push(`stage-4 baseline pin ${k} not set in manifest (fail-closed)`); continue; }
    if (!b || b.c !== pin.c || b.h !== pin.h) P.push(`stage-4 baseline digest ${k} {c:${b && b.c},h:${b && b.h}} != manifest pin {c:${pin.c},h:${pin.h}}`);
  }
  return P;
}
...
// ---- DEDICATED stage-7 re-anchor baseline (forward-only recovery from an APPLIED stage 7) -----------------
// A SEPARATE hard-coded artifact (anchorStage=7) governing ONLY the 8th migration (20260824, stageIdx 7) -- the
// forward correction that undoes 20260823. The stage-0/3/4/6 baselines are preserved untouched for audit. NOT
// generalized to arbitrary stages. buildStage7Baseline uses manifest-pinned values ONLY.
export function buildStage7Baseline({ head, fingerprint, pins = PROTECTED_DIGESTS }) {
  return {
    version: BASELINE_VERSION, projectRef: APPROVED_IDENTITY.projectRef, head, manifestFingerprint: fingerprint,
    migrationHashes: Object.fromEntries(MIGRATIONS.map((m) => [m.file, m.sha])), anchorStage: 7,
    protectedDigest: Object.fromEntries(PROTECTED_DIGEST_KEYS.map((k) => [k, pins[k]])), // manifest-pinned values ONLY
  };
}
// Validate the reviewed stage-7 baseline. Rejects: anchorStage != exactly 7; wrong project/head/fingerprint;
// migration-hash drift; edited/missing/extra/unpinned protected digest; and (on an APPLY) a migration whose
// stage precedes the anchor (stageIdx < 7 -- migrations 1-7 are NOT governed here).
export function validateStage7Baseline(baseline, { currentHead, currentFingerprint, envRef, stageIdx, pins = PROTECTED_DIGESTS } = {}) {
  const P = [];
  if (!baseline || typeof baseline !== "object") return ["stage-7 baseline malformed (not an object)"];
  if (baseline.version !== BASELINE_VERSION) P.push(`stage-7 baseline version ${baseline.version} != ${BASELINE_VERSION}`);
  if (baseline.anchorStage !== 7) P.push(`stage-7 baseline anchorStage ${baseline.anchorStage} != 7`);
  if (baseline.projectRef !== APPROVED_IDENTITY.projectRef) P.push(`stage-7 baseline projectRef ${baseline.projectRef} != approved`);
  if (envRef != null && baseline.projectRef !== envRef) P.push(`stage-7 baseline projectRef != current env ref ${envRef} (environment switched)`);
  if (baseline.head !== currentHead) P.push(`stage-7 baseline HEAD ${baseline.head} != current HEAD ${currentHead}`);
  if (baseline.manifestFingerprint !== currentFingerprint) P.push("stage-7 baseline manifest fingerprint stale");
  if (JSON.stringify(baseline.migrationHashes) !== JSON.stringify(Object.fromEntries(MIGRATIONS.map((m) => [m.file, m.sha])))) P.push("stage-7 baseline migration hashes differ from the manifest");
  if (stageIdx != null && stageIdx < 7) P.push(`stage-7 baseline cannot govern a migration at stage ${stageIdx} (precedes anchor 7)`);
  const pd = baseline.protectedDigest;
  if (!pd || typeof pd !== "object") { P.push("stage-7 baseline protectedDigest malformed"); return P; }
  if (!setEq(Object.keys(pd), PROTECTED_DIGEST_KEYS)) P.push(`stage-7 baseline digest key set {${Object.keys(pd).sort()}} != required {${[...PROTECTED_DIGEST_KEYS].sort()}}`);
  for (const k of PROTECTED_DIGEST_KEYS) {
    const pin = pins[k], b = pd[k];
    if (!pin) { P.push(`stage-7 baseline pin ${k} not set in manifest (fail-closed)`); continue; }
    if (!b || b.c !== pin.c || b.h !== pin.h) P.push(`stage-7 baseline digest ${k} {c:${b && b.c},h:${b && b.h}} != manifest pin {c:${pin.c},h:${pin.h}}`);
  }
  return P;
}
