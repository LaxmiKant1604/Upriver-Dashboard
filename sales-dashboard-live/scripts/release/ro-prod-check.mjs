...
import { verifyLedgerForStage, verifyStageObjects, verifyApprovedInvariants, captureProtectedDigest, compareProtectedDigest, requirePinnedStage0Digest, beginReadOnlySnapshot, buildBaseline, validateBaseline, shouldCreateBaseline } from "./release-state.mjs";
...
try {
  ...
  if (STAGE === 0) {
    // Blocker 2: the observed customer-data digest MUST equal the pinned Appendix AI value before a baseline exists.
    problems.push(...requirePinnedStage0Digest(digest));
  } else {
    let baseline = null;
    try { baseline = JSON.parse(readFileSync(baselinePath, "utf8")); } catch { problems.push("baseline file missing/malformed (run stage 0 first)"); }
    if (baseline) {
...
if (shouldCreateBaseline({ stage: STAGE, problemCount: problems.length, rolledBack })) {
  ...
  try {
    writeFileSync(baselinePath, JSON.stringify(baseline, null, 2), { flag: "wx" }); // exclusive create; never overwrite
    console.log(`BASELINE  created ${path.basename(baselinePath)} (head=${currentHead.slice(0, 12)} fp=${currentFingerprint.slice(0, 12)})`);
  } catch (e) {
