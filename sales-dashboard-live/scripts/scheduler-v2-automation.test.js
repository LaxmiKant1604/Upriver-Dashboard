...
test("D1. scheduler-v2.yml: dispatch-only, per-region concurrency, Node 24, >=90-min timeout", () => {
  ...
  // Cloudflare normally dispatches; scheduler-recovery is the sparse fallback. Native GitHub primaries are absent.
  const crons = [...yml.matchAll(/- cron:\s*"([^"]+)"/g)].map((m) => m[1]).sort();
  assert.deepEqual(crons, [], "scheduler-v2 has no native GitHub primary crons");
  ...
  assert.equal(groupLine.trim(), "group: scheduler-v2-${{ inputs.region }}", "all dispatchers use the same region key");
...
test("D3. paid scheduler-v2 is dispatch-only; scheduled workflows are zero-export workers; paid helper workflows stay manual-only", () => {
  const files = readdirSync(WORKFLOWS_DIR).filter((f) => f.endsWith(".yml") || f.endsWith(".yaml"));
  const scheduled = files.filter((f) => /\n\s*schedule:\s*\n/.test(readFileSync(resolve(WORKFLOWS_DIR, f), "utf8"))).sort();
  // scheduler-v2 is the sole automatic owner of every paid source but is dispatched externally. account-onboarding,
  ...
  assert.deepEqual(scheduled, ["account-onboarding.yml", "ads-publication-reconcile.yml", "fba-publication-reconcile.yml", "listing-health-v3-reconcile.yml", "oli-outbox-drain.yml", "oli-publication-reconcile.yml", "scheduler-recovery.yml", "scheduler-watchdog.yml"],
  ...
  // oli-outbox-drain (DR2 low-latency drain of the transactional outbox): exactly the operator entrypoint(s) (module
  // graph reaches no DataDoe export transport), no export/create/token/force-latest symbol anywhere -- the watchdog only
  // calls resume_stalled_catalog_job + finalize_sync_cycle RPCs; the drain runs the create-refusing reconciler +
  // the READ-ONLY outbox-monitor. Each entry lists ALL zero-export scripts the workflow may run (usually one).
  for (const [wf, ops] of [["oli-publication-reconcile.yml", ["node scripts/release/oli-publication-reconcile.mjs"]], ["fba-publication-reconcile.yml", ["node scripts/release/fba-publication-reconcile.mjs"]], ["ads-publication-reconcile.yml", ["node scripts/release/ads-publication-reconcile.mjs"]], ["listing-health-v3-reconcile.yml", ["node scripts/release/listing-health-v3-reconcile.mjs"]], ["scheduler-watchdog.yml", ["node scripts/release/scheduler-watchdog.mjs"]], ["oli-outbox-drain.yml", ["node scripts/release/oli-publication-reconcile.mjs", "node scripts/release/outbox-monitor.mjs"]]]) {
    ...
    assert.deepEqual([...new Set(reconcileCalls)].sort(), [...ops].sort(),
      "the " + wf + " workflow runs ONLY its zero-export operator script(s): " + JSON.stringify(reconcileCalls));
...
test("D4. previous-day (D-1) freshness shape: refresh_mode input, scheduled-always-normal, force-latest step (run_id, gated on mode + OLI-first), strict D-1 readiness + --strict-d1 release", () => {
  ...
  // scheduler-v2 is dispatch-only; Cloudflare + the GitHub recovery backstop own automatic timing.
  const crons = [...yml.matchAll(/- cron:\s*"([^"]+)"/g)].map((m) => m[1]).sort();
  assert.deepEqual(crons, [], "scheduler-v2 has no native GitHub cron");
});
