...
test("maximum SEVEN-day lookback: D-7 is used; D-8 is NOT", () => {
  const found = ENG.computeOliSalesEstimates({ accountId: ACC, operationalRows: [target("2026-08-29", 1)], referenceRows: [ref("2026-08-22", 70)], calculatedAt: "T" });
  assert.equal(found.estimates.length, 1);
  assert.equal(found.estimates[0].referenceDate, "2026-08-22"); // exactly 7 days back
  const tooOld = ENG.computeOliSalesEstimates({ accountId: ACC, operationalRows: [target("2026-08-29", 1)], referenceRows: [ref("2026-08-21", 70)], calculatedAt: "T" });
...
test("unresolved row is retained when NO trustworthy reference exists within 7 days", () => {
  const r = ENG.computeOliSalesEstimates({ accountId: ACC, operationalRows: [target("2026-08-29", 5)], referenceRows: [], calculatedAt: "T" });
...
test("re-running the SAME evidence produces byte-identical estimates (idempotent)", () => {
  const args = { accountId: ACC, operationalRows: [target("2026-08-29", 3)], referenceRows: [ref("2026-08-29", 100), ref("2026-08-28", 90)], calculatedAt: "T" };
...
test("provenance captured: reference date, unit price, method, reference request hash, quantity, timestamp", () => {
  const r = ENG.computeOliSalesEstimates({ accountId: ACC, operationalRows: [target("2026-08-29", 2)], referenceRows: [ref("2026-08-28", 55)], calculatedAt: "STAMP" });
...
test("adds the estimate to an EXISTING priced grain's sales_amount (units unchanged)", () => {
  ...
  const estimates = [{ accountId: ACC, sellerOrVendorId: SELLER, saleDate: "2026-08-29", sku: "SKU-A", childAsin: "B0ASIN", currency: "INR", estimatedSales: 150 }];
...
test("creates a SYNTHETIC row for a fully-unpriced grain (sales=estimate, units=0)", () => {
  const estimates = [{ accountId: ACC, sellerOrVendorId: SELLER, saleDate: "2026-08-29", sku: "SKU-A", childAsin: "B0ASIN", currency: "INR", estimatedSales: 120, referenceSourceRequestHash: "h" }];
...
test("ACTUAL supersedes ESTIMATE with NO double-count as itemization arrives", () => {
  ...
  const est1 = ENG.computeOliSalesEstimates({ accountId: ACC, operationalRows: [target("2026-08-29", 4, { priced_units: 1, pending_units: 4 })], referenceRows: [ref("2026-08-29", 100)], calculatedAt: "T" }).estimates;
  ...
  const est2 = ENG.computeOliSalesEstimates({ accountId: ACC, operationalRows: [target("2026-08-29", 0, { priced_units: 5, pending_units: 0 })], referenceRows: [ref("2026-08-29", 100)], calculatedAt: "T" }).estimates;
...
test("PARTIAL itemization: estimate covers ONLY the still-missing quantity (no double-count mid-way)", () => {
  ...
  const est = ENG.computeOliSalesEstimates({ accountId: ACC, operationalRows: [target("2026-08-29", 2, { priced_units: 3, pending_units: 2 })], referenceRows: [ref("2026-08-29", 100)], calculatedAt: "T" }).estimates;
...
test("resolvedEstimateGroupKeys marks resolved grains (for the missing-value breakdown)", () => {
  const est = [{ accountId: ACC, saleDate: "2026-08-29", sku: "SKU-A", childAsin: "B0ASIN", currency: "INR", estimatedSales: 10 }];
  const keys = ENG.resolvedEstimateGroupKeys(est);
  assert.equal(keys.has(ENG.oliGroupKey({ accountId: ACC, saleDate: "2026-08-29", sku: "SKU-A", childAsin: "B0ASIN", currency: "INR" })), true);
  assert.equal(keys.has(ENG.oliGroupKey({ accountId: ACC, saleDate: "2026-08-29", sku: "OTHER", childAsin: "B0ASIN", currency: "INR" })), false);
});
