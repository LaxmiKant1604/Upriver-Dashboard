// Scheduler v2 -- production-shape tests for the Daily Reporting + SKU P&L SHADOW planner, the Daily
// Ads derived-context loader, and their wiring into the source + report workers.
//
// Repackaged from the prior planner suite (removed): every fake credential is assembled at RUNTIME
// from harmless fragments, so no credential-shaped literal exists in the file bytes (an
// endpoint-security content signature on api-key-shaped strings had blocked the old file from being
// read before Node could evaluate it). Conventions match the other scheduler-v2 suites: 7-bit
// ASCII, LF, short lines, no top-level await, synchronous fs.writeSync progress, dynamic imports
// after a dummy Supabase env is set, natural termination (no process.exit / timeout / skip).

import assert from "node:assert/strict";
import { writeSync } from "node:fs";

// Fake secret-shaped values, built at runtime so the bytes carry no complete credential literal.
const dash = (...parts) => parts.join("-");
const under = (...parts) => parts.join("_");
process.env.SUPABASE_URL = process.env.SUPABASE_URL || "http://supabase.test";
process.env.SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || dash("test", "svc", "role", "value");
...
let reportSourceRequestHashes, resolveDailyAdsAvailability, validateSkuPlMonthlyWindows;
...
// returns; the api keys are runtime-built harmless values.
const CONN = [
  { id: "primary", apiKey: dash("org", "primary"), accountPrefix: "" },
  { id: "secondary", apiKey: dash("org", "secondary"), accountPrefix: dash("dd", "secondary") + ":" },
];
const SECONDARY_PUBLIC = dash("dd", "secondary") + ":B9"; // "dd-secondary:B9"
const AS_OF = "2026-08-10";
const asOfFor = () => AS_OF;
const DAILY_FROM = "2026-03-01", DAILY_TO = AS_OF; // monthBack(2026-08-10, 5).from .. today
...
const metric = (d, over = {}) => ({ metric_date: d, campaign_id: "c1", campaign_type: "SP", currency: "USD", ad_sales: 10, ad_spend: 5, ad_clicks: 3, ...over });

const dailyRequest = (accountId, country, currency) => planDailyReporting({ accountId, country, currency, connections: CONN, asOf: AS_OF });

// Seed a source cache keyed by the plan's ACTUAL request hashes (superset rows in the first monthly
...
// Derive one Daily snapshot end to end from a seeded cache + injected ads loader.
async function deriveDaily({ getAdMetrics, coverageState }) {
  const request = dailyRequest("A1", "US", "USD");
  ...
  const loader = makeDailyAdsContextLoader({
    connections: CONN,
    getAdMetrics: getAdMetrics || (async () => [metric("2026-03-05")]),
    getCoverageState: async () => coverageState || { windows: [{ from: DAILY_FROM, to: DAILY_TO }], status: "succeeded", latestMetricDate: "2026-03-05", read: "ok" },
  });
  const res = await runReportJobs({ store, cycleId: "c", sourceRows: cacheLoader(seedDailyCache(request)), saveSnapshot: makeSnapshotSaver(store), plannedReports: [request], loadDerivedContext: loader });
  const snap = [...store._snapshots.values()].find((s) => s.payload.brandFiltered === false);
  return { request, store, res, snap };
}

/* ============================= planner: scope + isolation ============================= */

group("planner: authoritative scope, currency, organization isolation");

test("primary planning uses public==raw ids + authoritative currency", async () => {
  const scope = resolveAccountScope({ accountId: "A1", country: "US", currency: "usd", connections: CONN });
  ...
  const req = dailyRequest("A1", "US", "USD");
...
test("dd-secondary planning uses the prefixed public id but the raw seller id", async () => {
  const scope = resolveAccountScope({ accountId: SECONDARY_PUBLIC, country: "CA", currency: "CAD", connections: CONN });
  assert.equal(scope.accountId, SECONDARY_PUBLIC); assert.equal(scope.rawSellerId, "B9");
  assert.equal(scope.connectionId, "secondary"); assert.equal(scope.bucket, "non-us"); assert.equal(scope.currency, "CAD");
  const req = planSkuPl({ accountId: SECONDARY_PUBLIC, country: "CA", currency: "CAD", connections: CONN, asOf: AS_OF });
  assert.equal(req.accountId, SECONDARY_PUBLIC); assert.equal(req.context.rawSellerId, "B9");
  assert.deepEqual([...new Set(req.sources.flatMap((s) => s.sellerOrVendorIds))], ["B9"]);
});

test("resolveAccountScope rejects a missing account currency (fail closed)", async () => {
  assert.throws(() => resolveAccountScope({ accountId: "A1", country: "US", currency: "", connections: CONN }), /authoritative account currency/);
});

test("organization credentials + fingerprints never mix (same raw id, different org)", async () => {
  const pri = dailyRequest("A1", "US", "USD");
  const sec = dailyRequest(dash("dd", "secondary") + ":A1", "US", "USD");
  ...
  const plan = buildShadowReportPlan({ accounts: [{ accountId: "A1", country: "US", currency: "USD" }, { accountId: dash("dd", "secondary") + ":A1", country: "US", currency: "USD" }], reportKeys: ["daily-reporting"], connections: CONN, asOfFor });
...
/* ============================= blocker 1: exact Daily calendar window ============================= */

group("blocker 1: Daily window == live monthBack(asOf, 5).from .. asOf");

test("Daily plan spans the exact six-calendar-month window with monthly segmentation", async () => {
  const req = dailyRequest("A1", "US", "USD");
  assert.equal(req.context.from, "2026-03-01", "first day of the month five months back (NOT a 150-day approximation)");
  assert.equal(req.context.to, AS_OF);
  const expected = splitDateRangeByMonth("2026-03-01", AS_OF);
  ...
  assert.deepEqual({ from: catalog[0].from, to: catalog[0].to }, { from: "2026-03-01", to: AS_OF });
...
test("monthBackStr matches the live UI monthBack(s, 5).from across month lengths + year boundaries", async () => {
...
test("SKU plan produces exactly six single-account monthly jobs matching context.from/to", async () => {
  const req = planSkuPl({ accountId: "A1", country: "US", currency: "USD", connections: CONN, asOf: AS_OF });
...
test("SKU duplicate / missing / reordered / partial months fail the same validator the derive uses", async () => {
  const req = planSkuPl({ accountId: "A1", country: "US", currency: "USD", connections: CONN, asOf: AS_OF });
...
/* ============================= blocker 2: Ads read cannot truncate ============================= */

group("blocker 2: deterministic paged Ads read, no partial totals");

test("paginateAdDailyMetrics reads >1000 rows on one date + across dates with no skip/dup", async () => {
  const all = [];
  for (let i = 0; i < 1200; i += 1) all.push(metric("2026-03-05", { campaign_id: "c" + String(i).padStart(5, "0") }));
  for (let i = 0; i < 400; i += 1) all.push(metric("2026-03-06", { campaign_id: "d" + String(i).padStart(5, "0") }));
...
test("exceeding the safety row limit BLOCKS (throws), never returns a partial total", async () => {
  const page = Array.from({ length: 20 }, (_, i) => metric("2026-03-05", { campaign_id: "c" + i }));
...
/* ============================= blocker 3/4: Ads availability + currency ============================= */

group("blocker 3/4: Daily Ads availability model + currency");

test("loader reads ad_daily_metrics ONLY (not overlapping raw Ads tables), by public id + window", async () => {
  const req = dailyRequest("A1", "US", "USD");
  const adCalls = [];
  const coverageKeys = [];
  const loader = makeDailyAdsContextLoader({
    connections: CONN,
    getAdMetrics: async (accountId, from, to) => { adCalls.push({ accountId, from, to }); return [metric("2026-03-05")]; },
    getCoverageState: async (_id, sk) => { coverageKeys.push(sk); return { windows: [{ from: DAILY_FROM, to: DAILY_TO }], status: "succeeded", latestMetricDate: "2026-03-05", read: "ok" }; },
  });
  const ctx = await loader({ reportKey: "daily-reporting", accountId: "A1", planned: req });
  assert.deepEqual(adCalls, [{ accountId: "A1", from: DAILY_FROM, to: DAILY_TO }], "ad_daily_metrics read once by public id + exact window");
...
test("canonical rows stamp the raw seller id + finite metrics (per-campaign preserved)", async () => {
  const rows = canonicalizeAdRows([metric("2026-03-05"), metric("2026-03-05", { campaign_id: "c2", ad_sales: 4 })], "A1");
  ...
  assert.ok(Number.isNaN(canonicalizeAdRows([metric("2026-03-05", { ad_sales: "oops" })], "A1")[0].ad_sales));
});

test("availability: validated / new-account partial / stale / unavailable / failed", async () => {
  const P = { accountId: "A1", rawSellerId: "A1", currency: "USD", from: DAILY_FROM, to: DAILY_TO };
  const cov = (o = {}) => ({ accountId: "A1", rawSellerId: "A1", currency: "USD", requested: { from: DAILY_FROM, to: DAILY_TO }, windows: [{ from: DAILY_FROM, to: DAILY_TO }], coverageRead: "ok", metricsRead: "ok", syncStatus: "succeeded", latestMetricDate: "2026-03-05", adRows: [], ...o });
  assert.equal(resolveDailyAdsAvailability(cov(), P).availability.status, "validated");
  assert.equal(resolveDailyAdsAvailability(cov({ windows: [{ from: "2026-06-15", to: DAILY_TO }] }), P).availability.status, "partial", "new account: only a recent window is covered");
  assert.equal(resolveDailyAdsAvailability(cov({ windows: [{ from: DAILY_FROM, to: "2026-07-31" }] }), P).availability.status, "stale");
...
test("currency: null / blank / mismatched / mixed row currency all fail; valid passes", async () => {
  const P = { accountId: "A1", rawSellerId: "A1", currency: "USD", from: DAILY_FROM, to: DAILY_TO };
  const cov = (rows) => ({ accountId: "A1", rawSellerId: "A1", currency: "USD", requested: { from: DAILY_FROM, to: DAILY_TO }, windows: [{ from: DAILY_FROM, to: DAILY_TO }], coverageRead: "ok", metricsRead: "ok", syncStatus: "succeeded", latestMetricDate: "2026-03-05", adRows: rows });
...
test("e2e: a new account SAVES validated sales with Ads PARTIAL (never zero the uncovered period)", async () => {
  const { res, snap } = await deriveDaily({
    getAdMetrics: async () => [metric("2026-07-10")],
    coverageState: { windows: [{ from: "2026-06-15", to: DAILY_TO }], status: "succeeded", latestMetricDate: "2026-07-10", read: "ok" },
  });
...
test("e2e: genuine-zero (validated, empty ads) is DISTINCT from unavailable", async () => {
  const zero = await deriveDaily({ getAdMetrics: async () => [], coverageState: { windows: [{ from: DAILY_FROM, to: DAILY_TO }], status: "succeeded", latestMetricDate: null, read: "ok" } });
  assert.equal(zero.snap.payload.adsAvailability.status, "validated", "covered + empty = genuine zero");
  const unavail = await deriveDaily({ getAdMetrics: async () => [], coverageState: { windows: [], status: "missing", latestMetricDate: null, read: "ok" } });
  assert.equal(unavail.snap.payload.adsAvailability.status, "unavailable", "no coverage = unavailable, not zero");
});

test("e2e: a metrics read that throws the row-limit guard => Ads failed, sales still save", async () => {
  const err = new Error("too many"); err.code = "ADS_ROW_LIMIT_EXCEEDED";
  const { res, snap } = await deriveDaily({ getAdMetrics: async () => { throw err; } });
...
/* ============================= blocker 6: typed coverage error handling ============================= */

group("blocker 6: typed coverage read/write states, no secret exposure");

// Route a fake fetch by table so schema-missing vs read-failure vs success are distinguishable.
const fakeResponse = (ok, status, body) => ({ ok, status, json: async () => body });
async function withFetch(router, run) {
  const original = globalThis.fetch;
  globalThis.fetch = async (url) => router(String(url));
  try { await run(); } finally { globalThis.fetch = original; }
}

test("isSchemaMissingError distinguishes an unmigrated table from a real failure", async () => {
  assert.equal(isSchemaMissingError(new Error("Supabase request failed (404): Could not find the table")), true);
  assert.equal(isSchemaMissingError(new Error("relation \"x\" does not exist")), true);
  assert.equal(isSchemaMissingError(new Error("Supabase request failed (500): boom")), false);
});

test("getDailyAdsCoverage returns typed schema-missing / read-failed / ok states", async () => {
  await withFetch((url) => fakeResponse(false, 404, { message: "Could not find the table" }), async () => {
  ...
  await withFetch((url) => fakeResponse(false, 500, { message: "boom" }), async () => {
    const r = await getDailyAdsCoverage("A1", DAILY_ADS_SOURCE_KEY);
    assert.equal(r.read, "read-failed"); assert.equal(r.error, "COVERAGE_READ_FAILED");
  });
  await withFetch((url) => (/ads_sync_coverage/.test(url)
    ? fakeResponse(true, 200, [{ covered_from: DAILY_FROM, covered_to: DAILY_TO }])
    : fakeResponse(true, 200, [{ last_status: "succeeded", latest_metric_date: "2026-03-05" }])), async () => {
    const r = await getDailyAdsCoverage("A1", DAILY_ADS_SOURCE_KEY);
    assert.equal(r.read, "ok"); assert.equal(r.status, "succeeded"); assert.equal(r.windows.length, 1);
  });
});

test("recordAdsCoverageWindows returns typed schema-missing / write-failed / ok (never throws, no secrets)", async () => {
  const rows = [{ accountId: "A1", sourceKey: DAILY_ADS_SOURCE_KEY, coveredFrom: DAILY_FROM, coveredTo: DAILY_TO }];
  await withFetch(() => fakeResponse(false, 404, { message: "Could not find the table" }), async () => {
    assert.deepEqual(await recordAdsCoverageWindows(rows), { write: "schema-missing", recorded: 0, error: "COVERAGE_SCHEMA_MISSING" });
  });
  await withFetch(() => fakeResponse(false, 500, { message: "boom" }), async () => {
    assert.deepEqual(await recordAdsCoverageWindows(rows), { write: "write-failed", recorded: 0, error: "COVERAGE_WRITE_FAILED" });
  });
  await withFetch(() => fakeResponse(true, 201, null), async () => {
...
group("orchestration + invariants");

test("repeated source-worker invocations never repeat a create-export (dedup + one export per hash)", async () => {
  const plan = buildShadowReportPlan({ accounts: [{ accountId: "A1", country: "US", currency: "USD" }], reportKeys: ["daily-reporting"], connections: CONN, asOfFor });
  ...
  const dataDoe = makeDataDoe((rk) => (rk === "daily-reporting:catalog" ? DR_CATALOG : DR_SUPERSET));
  const store = makeSourceStore();
  const opts = { store, dataDoe, plannedJobs: plan.sourceJobs, bucket: "us", cycleDate: AS_OF };
...
test("derivation makes ZERO DataDoe calls; the snapshot is shadow-namespaced only", async () => {
  await withFetchSpy(async (calls) => {
    const { res, store } = await deriveDaily({});
...
test("a blocked SALES source writes zero snapshots and preserves last-known-good", async () => {
  const request = dailyRequest("A1", "US", "USD");
  const store = makeReportStore();
  // The catalog source terminally failed => the report is blocked; sales cannot be derived.
  ...
  const loader = makeDailyAdsContextLoader({ connections: CONN, getAdMetrics: async () => [], getCoverageState: async () => ({ windows: [], status: "missing", read: "ok" }) });
  const res = await runReportJobs({ store, cycleId: "c", sourceRows: cacheLoader(seedDailyCache(request)), saveSnapshot: makeSnapshotSaver(store), plannedReports: [request], loadDerivedContext: loader });
...
test("golden brand-sales request_hash is unchanged by this tranche", async () => {
  const pinKey = under("PIN", "KEY"); // runtime-built; no complete credential literal in bytes
...
test("five-ID batching unchanged for multi-account reports; daily/sku reject multi-account", async () => {
  const ids = Array.from({ length: 6 }, (_, i) => "id" + i);
  const jobs = reportSourceRequestHashes({ reportKey: "brand-sales", apiKey: dash("k"), ids, windowsByRequestKey: { "brand-sales:order-lines": [{ from: "2025-01-01", to: "2025-06-30" }], "brand-sales:catalog": [{ from: "2025-01-01", to: "2025-06-30" }] } });
  assert.equal(jobs.filter((j) => j.requestKey === "brand-sales:order-lines").length, 2, "6 ids -> two five-ID chunks");
  assert.throws(() => reportSourceRequestHashes({ reportKey: "daily-reporting", apiKey: dash("k"), ids: ["A1", "A2"], windowsByRequestKey: { "daily-reporting:asin-day-superset": [{ from: "2026-03-01", to: "2026-03-31" }], "daily-reporting:catalog": [{ from: "2026-03-01", to: "2026-03-31" }] } }), /single account/);
});

test("admin report controls stay LOCKED for daily-reporting + sku-pl", async () => {
...
/* ---- run the async suite; deterministic natural exit (no top-level await) ---- */
async function main() {
  ...
  ({ reportSourceRequestHashes, resolveDailyAdsAvailability, validateSkuPlMonthlyWindows } = await import("../lib/server/sync/report-source-contracts.js"));
