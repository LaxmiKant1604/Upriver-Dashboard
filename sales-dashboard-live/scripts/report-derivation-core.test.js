// Scheduler v2 Phase 1d -- report-derivation CORE tests (SHADOW MODE).
//
// The 66 report-derivation assertions restored from the previously-readable c790f22 baseline as
// one small, independently-readable ESM artifact (split out of the former combined suite so the
// content scanner reads each file directly). Proves the derivation layer builds report snapshots
// PURELY from already-saved canonical source rows: zero DataDoe exports, idempotent +
// checkpointable, last-known-good preserved on failure, cache-miss never treated as empty
// success, truncated/failed sources never derived, terminal vs degraded policies,
// primary/dd-secondary isolation, derived-only reports own no source jobs, and an unchanged
// golden request_hash.
//
// 7-bit ASCII, LF, no top-level await, synchronous writeSync progress, dynamic imports after a
// dummy Supabase env. Nothing high-entropy in the bytes except the one golden request_hash pin.
...
let splitDateRangeByMonth, splitDateRangeByDays, canonicalOliSlices;
...
test("golden request_hash pins the corrected DataDoe OLI limit and unchanged Catalog identity", async () => {
  const jobs = reportSourceRequestHashes({
    reportKey: "brand-sales", apiKey: ["PIN", "KEY"].join("_"), ids: ["A1"],
...
function seedDailyCycle() {
  const store = makeMemoryReportStore();
  const slices = canonicalOliSlices(DR_FROM, DR_TO);
  ...
  const plannedReports = [plan("daily-reporting", "A1", [
    ...entries.map((e) => src("daily-reporting:oli-sales", e.hash, { sellerOrVendorIds: ["S1"], from: e.window.from, to: e.window.to })),
...
async function main() {
  ...
  ({ splitDateRangeByMonth, splitDateRangeByDays, canonicalOliSlices } = await import("../lib/server/date-windows.js"));
