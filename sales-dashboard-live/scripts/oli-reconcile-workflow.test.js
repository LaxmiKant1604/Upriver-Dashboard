...
ok("region work is isolated: a matrix over the 3 regions with bounded concurrency (max-parallel 1, fail-fast false)", /matrix:[\s\S]{0,80}bucket: \["india", "europe-au", "us-ca"\]/.test(wf) && /max-parallel: 1/.test(wf) && /fail-fast: false/.test(wf));
ok("per-region concurrency group, cancel-in-progress false (a delayed tick queues, never races)", /group: oli-publication-reconcile-\$\{\{ github\.event\.inputs\.bucket \|\| 'all' \}\}/.test(wf) && /cancel-in-progress: false/.test(wf));
ok("DEFAULT is dry-run: --live is added ONLY on a manual live dispatch OR when vars.OLI_RECONCILE_LIVE=='true'", /vars\.OLI_RECONCILE_LIVE == 'true' && 'live' \|\| 'dry-run'/.test(wf));
ok("scheduled/live runs pass --live; dry-run runs do NOT", /--live\b/.test(wf) && /--mode=periodic\b/.test(wf));
ok("it runs the reconciler entrypoint", /node scripts\/release\/oli-publication-reconcile\.mjs --bucket=\$\{\{ matrix\.bucket \}\}/.test(wf));
ok("as-of is a conservative D-1 (yesterday UTC)", /date -u -d 'yesterday' \+%F/.test(wf));
...
// ---- scheduler-v2.yml immediate post-save reconcile step (WORK 6) ----
...
const step = sched.slice(stepStart, stepStart + 900);
...
ok("it runs always() + only for a non-bootstrap OLI-success with eligible ids + an effective as-of", /if: always\(\) && steps\.cfg\.outputs\.scope != 'bootstrap' && steps\.oli\.outcome == 'success' && steps\.oli\.outputs\.eligible_ids != '' && steps\.readiness\.outputs\.effective_asof != ''/.test(step));
...
ok("it is DRY-RUN unless the repository variable OLI_RECONCILE_LIVE=='true'", /vars\.OLI_RECONCILE_LIVE .. = "true" .. LIVE="--live"/.test(step) || /if \[ "\$\{\{ vars\.OLI_RECONCILE_LIVE \}\}" = "true" \]; then LIVE="--live"/.test(step));
