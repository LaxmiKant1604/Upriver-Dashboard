...
- Brand View follow-up deployed as `dpl_9qBDTMBqZFTgyX7xNCnT7rpkZvLB` (`https://upriverdashboard.vercel.app`): Daily and Monthly remain free of Ad Spend/TACoS. The 7-Day Performance table restores brand-scoped, day-by-day Ad Spend and TACoS plus the 7-day total, using the existing saved Ads data and its coverage guardrails. The Monthly Snapshot already uses the latest saved month as its `Current Month Actual` and run-rate columns; it does not label missing future days as zero.
